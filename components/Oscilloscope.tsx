...
const Oscilloscope: React.FC<Props> = ({ simRef }) => {
  ...
  useEffect(() => {
    const render = () => {
      ...
      if (canvas && sim) {
        ...
        if (ctx) {
            const data = sim.noisyBuffer;
            const idealData = sim.idealBuffer;
            const maxAmp = sim.maxAmplitude;

            // Responsive Canvas
            const dpr = window.devicePixelRatio || 1;
            const rect = canvas.getBoundingClientRect();
            // Check if resize needed to avoid clearing needlessly if size hasn't changed
  ...
  }, [simRef]); // Only restart if ref object identity changes (which it shouldn't)
